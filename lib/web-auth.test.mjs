...
const { isValidBasicAuthorization, isWebPasswordEnabled, ROCINANTE_AUTH_USERNAME } = await jiti.import("./web-auth.ts");

test("accepts only the configured Basic Auth credentials", () => {
  const header = `Basic ${Buffer.from(`${ROCINANTE_AUTH_USERNAME}:secret`).toString("base64")}`;
