...
test("Rocinante update route permits only explicit check, update, and restart actions", async () => {
  const route = await readFile(new URL("../app/api/rocinante-update/route.ts", import.meta.url), "utf8");
...
test("settings groups runtime preferences and resource managers behind tabs", async () => {
  ...
  // After the i18n pass, the visible labels live in `lib/i18n/locales/*.json`
  // and SettingsConfig references them via t() — assert on the key paths,
  // not on the literal English strings (which would now only exist in en.json).
  assert.match(settings, /settings\.ompUpdate\.installButton/);
  assert.match(settings, /settings\.ompUpdate\.restartButton/);
  assert.match(settings, /settings\.advisor\.enableDesc/);
